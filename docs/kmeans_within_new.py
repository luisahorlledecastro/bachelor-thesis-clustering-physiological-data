...
class KM:
    def __init__(self):
        ...
        scr = SCR()
        self.data = scr.data_between
        self.data_var = scr.df_between
        self.data_clustering_var = scr.df_single_between
        self.data_clustering_no_var = scr.df_single_between_no_var
    ...
    def elbow_method(self, k_max):
        ...
        """
        self.data_clustering_no_var.columns = self.data_clustering_no_var.columns.astype(str)
        print(self.data_clustering_no_var)
        ...
        for k in K:
            ...
            kmeans = kmeans.fit(self.data_clustering_no_var)
    ...
    def kmeans_cluster(self, k):
        ...
        kmeans = KMeans(n_clusters=k)
        kmeans.fit(self.data_clustering_no_var)

        # Add cluster labels to the original DataFrame
        self.data_clustering_no_var['cluster'] = kmeans.labels_

        # Now you have cluster labels in the DataFrame, y
        # ou can separate the data into separate DataFrames for each cluster
        self.cluster_dfs = []
        for cluster_label in range(k):
            self.cluster_df = self.data_clustering_no_var[self.data_clustering_no_var['cluster'] == cluster_label].drop('cluster', axis=1)
            self.cluster_dfs.append(self.cluster_df)

        self.num_clusters = k

        self.k_metrics(kmeans, k)

        return None

    def k_metrics(self, model, k):
        """

        :return:
        """
        # Calculate clustering metrics
        self.silhouette = silhouette_score(self.data_clustering_no_var, model.labels_)
        self.db_index = davies_bouldin_score(self.data_clustering_no_var, model.labels_)
        self.ch_index = calinski_harabasz_score(self.data_clustering_no_var, model.labels_)
...
if __name__ == '__main__':
    ...
    km = KM()
    #km.elbow_method(40)
    km.k_evaluation((2, 20))
    print(km.cluster_dfs)
